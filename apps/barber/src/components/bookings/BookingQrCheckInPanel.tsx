...
export function BookingQrCheckInPanel({
  onCheckedIn,
  wide = false,
}: {
  onCheckedIn?: () => void;
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        wide ? "flex min-h-[min(520px,70vh)] flex-col p-8" : "p-6 sm:p-8",
      )}
    >
      <div className={cn("flex flex-col items-center text-center", wide && "shrink-0")}>
        <span
          className={cn(
        ...
        <h2 className="mt-4 font-heading text-xl font-semibold lg:text-2xl">Mijozni QR bilan tasdiqlang</h2>
      ...
      <div className={cn(wide && "mt-auto flex flex-1 flex-col justify-end")}>
        <button
          type="button"
          onClick={() => {
            setScannerOpen(true);
        ...
        <div className={cn("mt-4", wide ? "flex flex-col gap-3" : "flex gap-2")}>
          <div className={cn("relative min-w-0", wide ? "w-full" : "flex-1")}>
            <input
              ref={inputRef}
              defaultValue=""
              lang="en"
          ...
          <button
            ...
            disabled={busy}
            className={cn(
              "inline-flex items-center justify-center gap-1.5 rounded-xl border border-border bg-muted text-sm font-semibold transition-colors hover:bg-muted/80 disabled:opacity-60",
              wide
