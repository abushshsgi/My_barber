...
export function SalonHoursSection({
  ...
  return (
    <section className="space-y-4">
      <h2 className="text-xl font-bold tracking-tight">{t("salon.hours.title")}</h2>
      <div className="divide-y divide-border rounded-2xl border border-border">
        {[1, 2, 3, 4, 5, 6, 0].map((weekday) => {
          const row = byWeekday.get(weekday);
          const closed = closedWeekdays.includes(weekday);
          return (
            <div key={weekday} className="flex items-center justify-between px-4 py-3 text-sm">
              <span className="font-medium">{weekdayLabel(weekday, t)}</span>
              <span className={closed ? "text-muted-foreground" : "font-bold tabular-nums"}>
                {closed
