...
export function SettingsFieldRow({
  ...
  const actionClass =
    "shrink-0 text-sm font-semibold text-foreground underline underline-offset-2 transition-opacity hover:opacity-70 disabled:opacity-40";

  const resolvedActionLabel = isEditing
    ? (cancelLabel ?? "Bekor")
    : actionLabel;
    ...
    <div className={cn("border-b border-border py-6 last:border-b-0", isEditing && "pb-4")}>
      <div className="flex items-start justify-between gap-6">
        <div className="min-w-0 flex-1">
          ...
          {!isEditing ? <div className="mt-1 text-sm text-muted-foreground">{displayValue}</div> : null}
          {hint && !isEditing ? (
