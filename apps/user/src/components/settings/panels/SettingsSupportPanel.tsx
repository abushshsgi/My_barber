...
export function SettingsSupportPanel({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div className={embedded ? "mt-4 grid gap-6 lg:grid-cols-2 lg:items-start" : "grid gap-6 lg:grid-cols-2 lg:items-start"}>
      <div className="space-y-2">
        ...
        <ProfileSubpageCard className="border-foreground bg-foreground text-background">
          <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-background/70">Bog'lanish</p>
      ...
      <div>
        <p className="mb-4 text-sm text-muted-foreground">
          Tez-tez so'raladigan savollarni chapda toping yoki murojaat yuboring.
        </p>
            ...
            {(ticketsQ.data?.length ?? 0) > 0 ? (
              <ul className="mt-2 space-y-2 border-t border-border pt-3">
                {ticketsQ.data?.map((ticket) => (
                  <li key={ticket.id} className="rounded-lg bg-surface/50 px-3 py-2 text-sm">
