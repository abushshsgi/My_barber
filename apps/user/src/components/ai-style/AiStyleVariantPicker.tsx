...
import {
  ...
  AI_STYLE_PAGE_VARIANTS,
  aiStyleVariantMeta,
} from "@/components/ai-style/ai-style-variants";
import { cn } from "@/lib/utils";

type Props = {
  value: AiStylePageVariant;
  onChange: (variant: AiStylePageVariant) => void;
};

export function AiStyleVariantPicker({ value, onChange }: Props) {
  const { t } = useTranslation();
  const activeIndex = AI_STYLE_PAGE_VARIANTS.indexOf(value) + 1;

  return (
    <section className="mb-4 rounded-[24px] border border-dashed border-border bg-surface/60 p-4">
      <div className="flex items-center justify-between gap-2">
        <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground">
          {t("aiStylePage.variantPicker.title")}
        </p>
        <span className="shrink-0 rounded-full bg-foreground px-2 py-0.5 text-[10px] font-bold text-background">
          {activeIndex}/3
        </span>
      </div>
      <p className="mt-1 text-[11px] text-muted-foreground">{t("aiStylePage.variantPicker.subtitle")}</p>
      <div className="mt-3 grid grid-cols-3 gap-2">
        {AI_STYLE_PAGE_VARIANTS.map((id, index) => {
          ...
          return (
            ...
            >
              ...
              <span className="line-clamp-2 min-h-[28px] text-center text-[9px] font-bold leading-tight">
                {t(aiStyleVariantMeta[id].labelKey)}
              </span>
            </button>
          );
        })}
