...
/** Profil — obuna upgrade CTA (mobil + desktop). */
export function ProfileUpgradeButton({ className, variant = "pill" }: Props) {
  ...
  const label = isPro
    ...
    : hasActive
      ? t("profile.upgradeMore", { defaultValue: "Upgrade — ko‘proq ochish" })
      : t("profile.upgrade", { defaultValue: "Upgrade" });

  const plan = isPro ? "pro" : hasActive && code === "starter" ? "plus" : code === "plus" ? "pro" : "starter";
