...
export function BookingsMobile({ focus }: Props) {
  ...
  return (
    <div>
      ...
      <div className="sticky top-0 z-10 bg-background/95 px-5 backdrop-blur-md">
      ...
      <div className="px-5 pt-6">
        {isLoading ? (
          ...
          <EmptyState
            ...
            action={
              <Link to="/" className="rounded-2xl bg-foreground px-5 py-3 text-sm font-bold text-background">
                Salonlarni ko'rish
              </Link>
            }
          />
...
function BookingCard({ booking: b, focused }: { booking: BookingItem; focused?: boolean }) {
  ...
  return (
    <div
      id={`booking-${b.id}`}
      className={cn("rounded-2xl border border-border bg-background p-4", focused && "ring-2 ring-foreground")}
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            ...
            <span className={cn("shrink-0 rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase", statusStyles[b.status])}>
      ...
      <div className="mt-3 flex gap-2 border-t border-border pt-3">
        {b.status === "done" ? (
          <button type="button" className="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-surface py-2.5 text-xs font-bold">
            <Star className="h-3.5 w-3.5" /> {t("bookings.writeReview")}
          </button>
        ) : (
          <Link to="/chat" className="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-surface py-2.5 text-xs font-bold">
            <MessageSquare className="h-3.5 w-3.5" /> {t("bookings.chat")}
          </Link>
        )}
        {b.status === "pending" || b.status === "accepted" ? (
          <button type="button" className="flex-1 rounded-xl border-2 border-foreground py-2.5 text-xs font-bold">
            {t("common.cancel")}
          </button>
        ) : null}
      </div>
