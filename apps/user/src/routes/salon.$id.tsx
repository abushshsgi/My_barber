        ...
        <div className="no-scrollbar flex gap-1 overflow-x-auto px-3">
          {TAB_KEYS.map((k) => (
            <button
              ...
              onClick={() => setTab(k)}
              className={cn(
                "relative shrink-0 px-3 py-3.5 text-[12px] font-bold",
                tab === k ? "text-foreground" : "text-muted-foreground",
              )}
            >
              {t(`salon.tabs.${k}`)}
              {tab === k ? <span className="absolute inset-x-3 bottom-0 h-[3px] rounded-t-full bg-foreground" /> : null}
            </button>
          ))}
        </div>
      </div>
            ...
            {salon.services.map((s) => (
              <div key={s.id} className="flex items-center justify-between gap-3 py-4">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground">{s.duration} {t("salon.minutes")}</p>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-sm font-bold">{formatPrice(s.price)}</span>
                  <Link to="/booking/$salonId" params={{ salonId: salon.id }} className="grid h-9 w-9 place-items-center rounded-full bg-foreground text-background">
        ...
        {tab === "portfolio" && (
          <div className="grid grid-cols-3 gap-2">
            {salon.portfolio.map((p, i) => (
              <div key={p} className="aspect-square rounded-xl" style={{ background: `linear-gradient(${135 + i * 20}deg, oklch(0.85 0.04 ${(i * 60) % 360}), oklch(0.5 0.06 ${(i * 60 + 80) % 360}))` }} />
            ))}
          </div>
        )}
      </div>
...
function SalonPage() {
  return <DesktopPageSplit mobile={<SalonMobile />} desktop={<SalonDesktopPage />} />;
}
