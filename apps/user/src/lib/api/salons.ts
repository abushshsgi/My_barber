...
import type { ApiNearbySalon, ApiSalonDetail, ApiSalonList, ApiSalonStaff } from "./types";
...
export async function fetchSalons(params?: { ids?: string; region?: string }): Promise<ApiSalonList[]> {
  return apiJson<ApiSalonList[]>(`/api/v1/salons/${qs(params ?? {})}`);
}
...
export async function searchSalons(q: string): Promise<ApiSalonList[]> {
  return apiJson<ApiSalonList[]>(`/api/v1/salons/search/${qs({ q })}`);
}
