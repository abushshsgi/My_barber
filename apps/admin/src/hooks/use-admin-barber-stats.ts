...
import { fetchAdminBarberAnalytics } from "@/lib/admin-api";
import {
  buildAdminDailyRows,
  buildDailyBarChart,
  type StatsDailyRow,
...
export function useAdminBarberStatsMetrics(barberId: string, range: StatsRangeKey) {
  const dates = useMemo(() => statsRangeToIsoParams(range), [range]);
  const { data: analytics, isLoading, isError, isFetching } = useQuery({
    queryKey: ["admin", "barber", barberId, "analytics", dates.start, dates.end],
    queryFn: () => fetchAdminBarberAnalytics(barberId, dates),
    staleTime: 20_000,
  });
...
export type AdminBarberStatsMetrics = ReturnType<typeof useAdminBarberStatsMetrics>;
