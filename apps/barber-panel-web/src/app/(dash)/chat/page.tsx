...
import { useConversations, useConversationMessages, useSendMessage } from "@/queries/chat";
      ...
      <div className="flex h-[calc(100vh-3.5rem)]">
        <div className="w-80 border-r border-border bg-card">
          ...
          <div className="space-y-1 px-2 pb-3">
            ...
            {convos.isLoading && <div className="px-3 py-2 text-sm text-muted-foreground">Loading...</div>}
        ...
        <div className="flex flex-1 flex-col">
          <div className="flex-1 overflow-y-auto p-6">
            <div className="mx-auto max-w-2xl space-y-3">
              ...
              {messagesQ.isLoading && <div className="text-sm text-muted-foreground">Loading messages...</div>}
              {!messagesQ.isLoading && selectedId && messages.length === 0 && (
                <div className="text-sm text-muted-foreground">No messages.</div>
              )}
              {!selectedId && <div className="text-sm text-muted-foreground">Select a conversation.</div>}
            </div>
          </div>
