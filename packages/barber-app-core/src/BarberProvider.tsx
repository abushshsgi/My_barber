...
import { inferFlowIdentity, type FlowIdentity } from "@mybarber/barber-shared";
import type { BarberApi, UploadFile } from "./api";
...
export type Booking = {
  ...
  service: string;
  date: string; // "Bugun" | "Ertaga" | "DD MMM"
  time: string;
  duration_min: number;
  price: number;
...
type Ctx = {
  ...
  /** WebSocket orqali kelgan xabar — ro‘yxat va ochiq threadni yangilash */
  applyIncomingChatMessage: (
    conversationId: string,
    msg: { id: number; sender_kind: "USER" | "BARBER"; text: string; created_at: string },
  ) => void;
  ...
  uploadPortfolio: (payload: { file: UploadFile; title: string; service: string }) => Promise<boolean>;
  addSalonImage: (payload: { file: UploadFile }) => Promise<boolean>;
...
export function BarberProvider({
  ...
  const { apiFetch, apiJson, apiList, clearTokens } = api;
    ...
    setConversations((prev) =>
      prev.map((c) => {
        ...
        if (msgs.some((m) => m.id === mapped.id)) {
          return { ...c, preview: mapped.text, time: mapped.time };
        }
        return {
          ...c,
          preview: mapped.text,
          time: mapped.time,
          messages: [...msgs, mapped],
        };
      }),
    );
  }, []);

  const applyIncomingChatMessage = useCallback(
    (
      conversationId: string,
      msg: { id: number; sender_kind: "USER" | "BARBER"; text: string; created_at: string },
    ) => {
      ...
      setConversations((prev) =>
        prev.map((c) => {
          if (c.id !== conversationId) return c;
          const msgs = c.messages ?? [];
          if (msgs.some((m) => m.id === mapped.id)) return c;
          return {
            ...c,
            preview: mapped.text,
            time: mapped.time,
  ...
  const addExpenseApi = useCallback(
    async (e: Omit<Expense, "id">) => {
      const spentOn = /^\d{4}-\d{2}-\d{2}$/.test(e.date)
        ? e.date
  ...
  const uploadPortfolioApi = useCallback(
    async (payload: { file: UploadFile; title: string; service: string }) => {
      const body = new FormData();
      body.append("image", payload.file as Blob);
  ...
  const addSalonImageApi = useCallback(
    async (payload: { file: UploadFile }) => {
      if (!salon.id) return false;
      const body = new FormData();
      body.append("images", payload.file as Blob);
  ...
  const value = useMemo<Ctx>(
    () => ({
      ...
      rejectBooking: (id) => void mutateBooking(id, "reject"),
