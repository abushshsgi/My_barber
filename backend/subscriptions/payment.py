...
@transaction.atomic
def pay_with_wallet(*, user: User, plan_code: str, request=None) -> UserSubscription:
    plan = get_plan(plan_code)
    if not plan:
        raise WalletServiceError("Noto'g'ri obuna rejasi.")

    price = Decimal(plan["price_uzs"])
    ...
    try:
        entry = WalletService.post_entry(
            ...
            metadata={"plan_code": plan_code, "source": "wallet"},
        )
    ...
    payment = SubscriptionPayment.objects.create(
        ...
        metadata={"wallet_entry_id": str(entry.pk)},
    )
    ...
    log_event(
        ...
        subscription=sub,
        actor=f"user:{user.pk}",
        detail={"provider": "wallet", "amount": str(price), "order_id": order_id},
        request=request,
    )
    return sub
...
def start_provider_checkout(
    *,
    ...
    payment = SubscriptionPayment.objects.create(
        ...
        metadata={"configured": result.configured, "message": result.message},
    )
    log_event(
